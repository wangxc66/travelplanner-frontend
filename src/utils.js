...
/**
 * 只有 401 代表「这个请求没有有效身份」。后端的 SecurityConfig 明确分开了两者：未登录走
 * authenticationEntryPoint 回 401，登录了但无权访问走 accessDeniedHandler 回 403。
 *
 * 403 一律不当作会话失效。它可能是越权访问别人的行程，也可能是 CORS 或配置问题 —— 把这些
 * 当成登录过期，会在一个和身份无关的错误上把用户踢回登录页，白丢他的进度。
 *
 * 早先后端没有配 exceptionHandling，所有鉴权失败都落到 Spring Security 的默认行为上，一律
 * 回 403，这里才不得不把 403 也算进来。现在没这个必要了。
 */
const isRejected = (status) => status === 401;

/**
 * 开发用的是内存数据库，后端一重启用户就没了，浏览器却还留着一个签名完好的 token —— 这种 401
 * 才是真的会话失效。但并发竞态同样会拿到 401（token 刚好在两个请求之间过期），所以先探测一次
 * 再决定要不要清。同时只探测一次：一批注定失败的请求不该变成一批探测请求。
 */
